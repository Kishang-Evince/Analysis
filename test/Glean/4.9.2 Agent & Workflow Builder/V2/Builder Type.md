...
- For Sr No 7-10 only: a terminal with Python 3.10+ and `pip` installed, and a Glean API token (Admin Console → look for API/developer token settings). **These rows are inherently developer-facing** - they're specifically testing the code-first/SDK build paths, which by definition need code to test. If you're not a developer, ask one on your team to run Sr No 7-10 and share the results, or mark them `Blocked (no developer available)`.
