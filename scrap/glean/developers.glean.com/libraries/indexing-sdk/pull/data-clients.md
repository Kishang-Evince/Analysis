---
url: "https://developers.glean.com/libraries/indexing-sdk/pull/data-clients"
canonical: "https://developers.glean.com/libraries/indexing-sdk/pull/data-clients"
title: "Data clients | Glean Developer"
description: "BasePullHttpStreamingDataClient - paginated HTTP data clients with no boilerplate"
fetched_at: "2026-09-01T13:23:04.692Z"
---
On this page

A data client is the source-facing half of a connector. At minimum it implements `get_source_data()`:

```
from glean.indexing.connectors import BaseDataClientclass WikiDataClient(BaseDataClient[WikiPage]):    def get_source_data(self, since=None, **kwargs):        return fetch_all_pages()
```

That's fine for small sources. For a paginated HTTP API, the SDK gives you a base class that already knows how to page.

## BasePullHttpStreamingDataClient[​](#basepullhttpstreamingdataclient "Direct link to BasePullHttpStreamingDataClient")

`BasePullHttpStreamingDataClient` combines [`PullHttpClient`](/libraries/indexing-sdk/pull/http-client) with [pagination](/libraries/indexing-sdk/pull/pagination) and yields records one at a time. For a well-behaved JSON API you often need no method body at all:
...
Invalid combinations fail at construction, not mid-crawl: offset pagination without a positive `page_size` raises `ValueError`, as does a negative `max_items`.

## Extracting records[​](#extracting-records "Direct link to Extracting records")
...
Everything above is a starting point. Override `get_source_data()` when your source needs something the base class doesn't model, and reuse `self.http`:
...
```
try:    connector.index_data(mode=IndexingMode.FULL)finally:    data_client.close()
```

## Limiting work during development[​](#limiting-work-during-development "Direct link to Limiting work during development")

`max_items` caps the number of records fetched, which keeps iteration fast while you're still shaping `transform()`:

```
ArticleDataClient(token, max_items=25)
```

The [test harness](/libraries/indexing-sdk/testing/integration) applies its own per-client `max_items` from `TestConfig`, so you don't have to hardcode this for tests.
