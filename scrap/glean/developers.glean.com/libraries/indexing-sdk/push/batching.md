---
url: "https://developers.glean.com/libraries/indexing-sdk/push/batching"
canonical: "https://developers.glean.com/libraries/indexing-sdk/push/batching"
title: "Batching and throughput | Glean Developer"
description: "Tuning batch size, byte limits, timeouts, and upload concurrency in the Glean Indexing SDK"
fetched_at: "2026-09-01T13:23:04.960Z"
---
On this page
...
## Find the bottleneck first[​](#find-the-bottleneck-first "Direct link to Find the bottleneck first")
...
Batches close on whichever limit hits first: `batch_size` documents or `max_batch_bytes` serialized bytes.

```
connector = WikiConnector(name="wiki", data_client=client)connector.batch_size = 250
```

Set the byte cap for a connector-managed run with the exact `ConnectorOptions` field:

```
connector.index_data(    options=ConnectorOptions(document_batch_size_bytes=2 * 1024 * 1024))
```

The connector forwards that value to byte-aware batching for in-memory, sync streaming, and async streaming uploads. Custom connectors can override the static `_resolve_max_batch_bytes(options)` hook when the cap must be selected at runtime; the hook's result is the effective cap.
...
```
# Give large batches longer.connector.index_data(options=ConnectorOptions(upload_timeout_ms=120_000))# Or make the batches smaller.connector.batch_size = 250
```
