---
url: "https://www.glean.com/connectors/google-drive"
canonical: "https://www.glean.com/connectors/google-drive"
title: "Google Drive integration with Glean | Glean Work AI"
description: "Enable secure, seamless Google Drive integration with Glean. Connect, search, and manage Docs, Sheets, and Slides directly from your workhub."
fetched_at: "2026-09-01T13:25:46.238Z"
---
### Overview

Glean's integration with Google Drive lets users search files alongside company knowledge, then take action — create and update Docs with context-rich outlines, build and format Sheets from tables and lists, and export content directly without manual copy-paste. All grounded in permissions-aware enterprise context.

### Features

Glean understands and retrieves:

-   Folders
-   Documents
-   Native file types such as Docs, Sheets, and Slides
-   Supported files in GDrive ([listed here](https://support.google.com/drive/answer/37603?hl=en))

Glean executes actions across:

-   Create and update Google Docs
-   Export content and outlines to Docs without copy-paste
-   Create and update Google Sheets, including tabs, ranges, and formatting
-   Export tables to Sheets for tracking, reporting, and analysis

### Benefits

Search Teams threads alongside project docs, tickets, and the rest of your company knowledge to understand what's happening and why. Send messages, post to channels, and keep every stakeholder aligned, without leaving Glean.

## Work AI that works.

[Get a demo](/get-a-demo)

![](https://cdn.prod.website-files.com/6127a84dfe068e153ef20572/69cb2e8bf65178f31d8136e2_career_hero-cta.webp)![](https://cdn.prod.website-files.com/6127a84dfe068e153ef20572/69cb2e8bf65178f31d8136e2_career_hero-cta.webp)![CTA Background Gradient 2](https://cdn.prod.website-files.com/6127a84dfe068e153ef20572/686c170144dca2c694924bde_40106bcbd58ff05381b4a609edc8f248_Group%202083936655-1.webp)![CTA Background Gradient 2 - Mobile](https://cdn.prod.website-files.com/6127a84dfe068e153ef20572/686d18a0442119d801986e9f_49f6945b6b449aeffda727e038aa97cf_CTA%20Background%20Gradient%202%20-%20Mobile.webp)![CTA Background Gradient 3](https://cdn.prod.website-files.com/6127a84dfe068e153ef20572/686c0ef31e585c1c0c9eb2a0_CTA%20Background.webp)![CTA Background Gradient 3](https://cdn.prod.website-files.com/6127a84dfe068e153ef20572/686c0ef31e585c1c0c9eb2a0_CTA%20Background.webp)![CTA Background Mobile](https://cdn.prod.website-files.com/6127a84dfe068e153ef20572/686cd671fb4cd509db55077c_CTA%20Background%20Mobile.webp)
