---
url: "https://docs.glean.com/connectors/native/sharepoint/security/api"
canonical: "https://docs.glean.com/connectors/native/sharepoint/security/api"
...
fetched_at: "2026-09-01T13:29:41.456Z"
---
On this page
...
Get the profile photo of a user. | User.Read.All | `https://graph.microsoft.com/v1.0/users/<user_id>/photo/$value` |
...
Get the default site groups and associated user memberships for a given site from the SharePoint REST API. | Sites.FullControl.All | `https://<site_domain>.sharepoint.com/sites/<subsite_url>/_api/web/SiteGroups?$expand=Users` |
...
Get the items within a list for a site using the SharePoint REST API. The SharePoint REST API is used as some content for classic sites is not available via Graph API. | Sites.FullControl.All | `https://<site_domain>.sharepoint.com/sites/<subsite_url>/_api/web/lists('<list_id>')/item` |
...
Get the web parts on a particular page (e.g. blocks of content within text boxes, titles, etc.) using the SharePoint REST API. | Sites.FullControl.All | `https://<site_domain>.sharepoint.com/sites/<subsite_url>/_api/web/GetFileById('<id>')/GetLimitedWebPartManager(scope=1)/ExportWebPart` |
...
List all the items within a drive (change-based, as per Microsoft's scanning guidance) | Sites.FullControl.All | `https://graph.microsoft.com/v1.0/drives/<drive_id>/root/delta` |
...
Glean subscribes to the \`driveItem\` resource which requires (as least privilege) the \`Files.ReadWrite.All\` permission to create the subscription. | Files.ReadWrite.All | `https://webhook.azurewebsites.net/api/send/<client>` |
